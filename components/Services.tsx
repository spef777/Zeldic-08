       ...
       <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {servicesList.map((service, index) => (
            <div 
              key={index}
              className="glass-panel p-8 rounded-2xl hover:bg-white/5 transition-all hover:-translate-y-2 duration-300 group cursor-default"
            >
              <div className="w-14 h-14 rounded-xl bg-violet-600/20 flex items-center justify-center text-violet-300 mb-6 group-hover:bg-violet-600 group-hover:text-white transition-colors">
                {service.icon}
              </div>
